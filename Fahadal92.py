...
# ✅ إصلاح ١: prefetch_all بالـ threads — بدل ساعة يخلص في دقائق
...
"""✅ للاستخدام في handle_check5 — بعد إغلاق الشمعة مباشرة، لا يحذف آخر شمعة"""
if df.empty:
return pd.DataFrame()
return (df.copy().set_index("ts")
.resample(f"{minutes}min", closed="left", label="left", origin=EPOCH)
...
# ✅ resample_ohlcv_closed — بدون iloc[:-1] لأننا بعد الإغلاق مباشرة
...
# Donchian — نقارن مع الـ rolling بدون shift لأن iloc[-1] هي المغلقة
