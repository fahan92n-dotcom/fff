...
def market_bias_osc(o, h, l, c, ha_len=50, ha_smooth=10):
    ...
    h_e = ema_tv(h, ha_len)
    l_e = ema_tv(l, ha_len)
    haclose = (o_e + h_e + l_e + c_e) / 4.0
    xhaopen = (o_e + c_e) / 2.0
    x = xhaopen.to_numpy()
    hc = haclose.to_numpy()
    oe = o_e.to_numpy()
    ce = c_e.to_numpy()
    ho = np.full(len(x), np.nan)
    for i in range(len(x)):
        if i == 0 or not np.isfinite(x[i - 1]) or not np.isfinite(hc[i - 1]):
            if np.isfinite(oe[i]) and np.isfinite(ce[i]):
                ho[i] = (oe[i] + ce[i]) / 2.0
        else:
...
def indicators_for(tfd):
    o, c, h, l = tfd["open"], tfd["close"], tfd["high"], tfd["low"]
...
def run_engine(n, cancel, mainNew, s1, s2, s3, s4, s5, s6mb, s7don, entryNew,
