...
# ✅ تقليل حجم الكاش لتوفير الـ RAM
FAST_FETCH_CANDLES = {"1m": 2000, "60m": 200}
API_FETCH_CANDLES = {"1m": 8000, "60m": 1000}
CACHE_MAX_CANDLES = {"1m": 9000, "60m": 1200}
...
# ──────────────────────────────────────────────
# ✅ التحميل بالخلفية — سريع أولاً ثم كامل بهدوء
# ──────────────────────────────────────────────
def prefetch_all(symbols):
"""تحميل سريع أولاً → البوت يبدأ يشتغل → تحميل كامل بالخلفية بهدوء"""

# المرحلة 1: تحميل سريع بـ 8 threads فقط (بدل 15)
def fetch_sym_fast(sym):
for tf, n in FAST_FETCH_CANDLES.items():
try:
df = get_ohlcv_full(sym, tf, target=n)
cache_merge(sym, tf, df)
time.sleep(0.1) # ✅ استراحة صغيرة تمنع الـ rate limit
except Exception as e:
log.warning(f"fast fetch {sym} {tf}: {e}")

log.info("🚀 بدء التحميل السريع...")
with ThreadPoolExecutor(max_workers=8) as ex:
ex.map(fetch_sym_fast, symbols)

fast_prefetch_done.set()
send_telegram("⚡ <b>التحميل السريع اكتمل — البوت يعمل الآن!</b>")
log.info("✅ التحميل السريع اكتمل")

# المرحلة 2: تحميل كامل بـ 5 threads فقط وبهدوء (بدل 10)
def fetch_sym_full(sym):
for tf, n in API_FETCH_CANDLES.items():
try:
df = get_ohlcv_full(sym, tf, target=n)
cache_merge(sym, tf, df)
time.sleep(0.3) # ✅ استراحة أطول في التحميل الكامل
except Exception as e:
log.warning(f"full fetch {sym} {tf}: {e}")

log.info("📦 بدء التحميل الكامل بالخلفية...")
# ✅ تحميل العملات واحدة واحدة بشكل دوري — ما يضغط على الـ RAM دفعة واحدة
for i, sym in enumerate(symbols):
fetch_sym_full(sym)
if (i + 1) % 10 == 0:
log.info(f"📦 تحميل كامل: {i+1}/{len(symbols)}")
time.sleep(2) # ✅ استراحة كل 10 عملات

prefetch_done.set()
send_telegram("✅ <b>التحميل الكامل اكتمل وجاهز للعمل!</b>")
log.info("✅ التحميل الكامل اكتمل")
...
log.warning(f"update {sym} {tf}: {e}")
# ✅ تقليل الـ workers من 30 إلى 10
...
hh = df5["high"].rolling(20).max()
ll = df5["low"].rolling(20).min()
don_green = bool(df5["close"].iloc[-1] > hh.iloc[-2])
don_red = bool(df5["close"].iloc[-1] < ll.iloc[-2])
if don_green:
don_color = "🟢 أخضر (صاعد)"
elif don_red:
don_color = "🔴 أحمر (هابط)"
...
if wait < -60:
log.warning(f"⚠️ check5_watcher تأخر {abs(wait):.0f}ث — تخطي")
...
# ✅ تأخير عشوائي عند البداية لمنع كل الـ threads تشتغل بنفس الوقت
time.sleep(entry_min * 2)
while True:
time.sleep(30)
if not fast_prefetch_done.is_set():
continue
...
# ✅ إصلاح: يشتغل حتى لو diag_counts["total"] == 0
msg = build_diag_msg(reset=False) if diag_counts["total"] > 0 else (
"⏳ البوت لسه ما بدأ الفحص — انتظر بعد اكتمال التحميل السريع"
)
...
# ✅ التحميل يبدأ مرة واحدة فقط عند أول تشغيل
if first_run and not fast_prefetch_done.is_set():
first_run = False
threading.Thread(
target=prefetch_all,
