...
FAST_FETCH_CANDLES = {"1m": 2000, "60m": 300}
API_FETCH_CANDLES  = {"1m": 2000, "60m": 300}
CACHE_MAX_CANDLES  = {"1m": 2500, "60m": 400}
...
def resample_ohlcv(df, minutes):
    """للاستخدام في scan_symbol — يحذف آخر شمعة لأنها قد تكون ناقصة"""
    if df.empty:
        return pd.DataFrame()
...
def resample_ohlcv_closed(df, minutes):
    """للاستخدام في handle_check5 — بعد إغلاق الشمعة، لا يحذف آخر شمعة"""
    if df.empty:
        return pd.DataFrame()
    return (df.copy().set_index("ts")
            .resample(f"{minutes}min", closed="left", label="left", origin=EPOCH)
...
def handle_check5(chat_id, symbol="BTCUSDT"):
    ...
    try:
        ...
        # ✅ فلتر عمر الشمعة — إذا آخر شمعة قديمة أكثر من 10 دقايق أعد الجلب
        now             = datetime.now(timezone.utc)
        ...
        # ✅ إذا الشمعة الأخيرة لم تُغلق بعد، احذفها
        ...
        # ── قراءة المؤشرات من آخر شمعة مغلقة ──
...
# ──────────────────────────────────────────────
# ✅ check5_watcher — إصلاح التأخر
# ──────────────────────────────────────────────
def get_next_close(tf_minutes):
    now   = datetime.now(timezone.utc)
    epoch = datetime(1970, 1, 1, tzinfo=timezone.utc)
    # ✅ استخدم total_seconds() بدون int() لتجنب خطأ الدقيقة الكسرية
...
def check5_watcher():
    while True:
        try:
            ...
            # ✅ لو تأخرنا أكثر من دقيقة — اقفز للشمعة التالية مباشرة
            if wait < -60:
            ...
            # ✅ انتظر حتى إغلاق الشمعة + 5 ثواني (بدل 2) لضمان استقرار Binance API
            time.sleep(max(wait, 0) + 5)

            if not fast_prefetch_done.is_set():
                continue

            # ✅ شغّل في thread منفصل حتى لا يعطل حلقة الانتظار
            threading.Thread(
