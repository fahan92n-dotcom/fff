...
def _macd_features(df_1m, minutes):
    """MACD histogram sign: green = line above signal, red = signal above line."""
    df = resample_ohlcv_closed(df_1m, minutes)
    if df.empty or len(df) < WARMUP_MACD:
        return None
...
def _scan_side(side, stepped, entry_data, grid, start, end, raw_1m, symbol):
    ...
    for level_idx, (main, confirm, entry, win_pct, loss_pct, group) in enumerate(LEVELS):
        ...
        owned = (active == level_idx) & confirm_macd
        ...
        for row_i, candle_end in enumerate(ends):
            ...
            green = bool(entry_df["macd_green"].iloc[row_i])
            red = bool(entry_df["macd_red"].iloc[row_i])
...
def scan_symbol(symbol, *, days=MONTH_DAYS, now=None, raw_1m=None):
    """Scan one symbol over the last ``days``. No EMA60/RSI/Donchian."""
...
def format_report(result):
    if not result.get("ready"):
        return ["⚠️ تعذر فحص تشبع SMI + MACD."]
...
def main():
    ...
    log.info("Scanning %s for %s days (SMI+MACD)...", ",".join(SYMBOLS), MONTH_DAYS)
