...
TELEGRAM_TOKEN = os.environ.get(
    "TELEGRAM_TOKEN", "8988740597:AAE_I7M7zB5VM2NykUwreQQMws0vk7qlU78"
)
...
def send_diag_report():
    """Send diagnostics report every hour."""
    while True:
        time.sleep(3600)
...
def delete_webhook():
    ...
    try:
        ...
        log.error("deleteWebhook error: %s", e)
...
def get_report(period="today"):
    """Get trading signals report for period."""
    now = datetime.now(timezone.utc)
...
def get_ohlcv(symbol, tf, limit=500):
    ...
    try:
        ...
        log.error("get_ohlcv %s %s: %s", symbol, tf, e)
    return pd.DataFrame()


def get_ohlcv_full(symbol, tf, target):
    """Fetch full OHLCV data from Binance with retries."""
    binance_tf = TF_MAP.get(tf, "1m")
...
def cache_merge(symbol, tf, new_df):
    """Merge new OHLCV data into cache."""
    if new_df.empty:
        return
    key = (symbol, tf)
...
def prefetch_all(symbols):
    """Prefetch OHLCV data for all symbols."""
...
def cache_updater_1m():
    """Update 1m candle cache periodically."""
    while True:
...
def cache_updater_60m():
    """Update 60m candle cache periodically."""
    while True:
        time.sleep(3600)
...
def resample_ohlcv(df, minutes):
    """Resample OHLCV to specified timeframe (open candle)."""
    if df.empty:
        return pd.DataFrame()
    return (
        df.copy()
...
def resample_ohlcv_closed(df, minutes):
    """Resample OHLCV to specified timeframe (closed candles)."""
    if df.empty:
        return pd.DataFrame()
    return (
        df.copy()
...
def wilder_rma(series, period):
    """Calculate Wilder's RMA (EMA with alpha=1/period)."""
    return series.ewm(alpha=1.0 / period, min_periods=period, adjust=False).mean()


def _calc_macd_hist(close):
    """Calculate MACD histogram."""
    ml = close.ewm(span=12, min_periods=12, adjust=False).mean() - close.ewm(
        span=26, min_periods=26, adjust=False
    ).mean()
...
def _calc_macd_full(close):
    ...
    macd_line = close.ewm(span=12, min_periods=12, adjust=False).mean() - close.ewm(
        span=26, min_periods=26, adjust=False
    ).mean()
...
def check_macd_red(df):
    """Check if MACD histogram is red (negative)."""
...
def check_macd_green(df):
    """Check if MACD histogram is green (positive)."""
    if len(df) < WARMUP_MACD:
        return False
    return bool(_calc_macd_hist(df["close"]).iloc[-1] > 0)
...
def calc_donchian_trend(df, length=20):
    """Calculate Donchian trend (1=uptrend, -1=downtrend, 0=neutral)."""
    if len(df) < length + 2:
        return []
...
def check_donchian_ribbon(df, direction="green"):
    """Check Donchian Ribbon direction."""
    if len(df) < 22:
        return False
...
def get_smi_value(df):
    """Get current SMI value and signal."""
    if len(df) < WARMUP_SMI:
        return None, None
...
def calc_rsi_tv(close, period=14):
    """Calculate RSI (TradingView style)."""
    delta = close.diff()
...
def calc_stoch_tv(close, high, low, k_len=15, k_smooth=3, d_smooth=3):
    """Calculate Stochastic (TradingView style)."""
...
def check_rsi_stoch(df, lookback=5):
    """Check for RSI/Stochastic crossover."""
...
def handle_check5(chat_id, symbol="BTCUSDT"):
    """Handle /check5 command to generate technical analysis report."""
    ...
    try:
        ...
        if age_minutes > 10:
            try:
                log.warning("Data stale, refreshing...")
                df_fresh = get_ohlcv(symbol, "1m", limit=1000)
                if not df_fresh.empty:
                    cache_merge(symbol, "1m", df_fresh)
                df_raw = get_cached(symbol, "1m")
...
def check5_watcher():
    """Watch for 5m candle close and generate reports."""
    while True:
        try:
            ...
            threading.Thread(
                target=handle_check5, args=(TELEGRAM_CHAT_ID, "BTCUSDT"), daemon=True
            ).start()

        except Exception as e:
            log.error("check5_watcher error: %s", e)
            time.sleep(10)
...
def candle_watcher(entry_min, confirm_min, third_min, ec_api, t_api):
    """Watch candles and scan for signals."""
    while True:
        time.sleep(30)
        if not fast_prefetch_done.is_set():
            continue
...
def update_symbols_loop():
    """Update top symbols by volume periodically."""
    while True:
        try:
            ...
            log.error("update_symbols_loop: %s", e)
        time.sleep(3600)
