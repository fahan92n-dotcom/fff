...
def check_macd_red(df):
    """✅ يستخدم [-1] لأن resample_ohlcv يضمن أن الأخيرة مغلقة"""
...
def check_donchian_ribbon(df, direction="green"):
    """
    ✅ إصلاح: يستخدم [-1] وليس [-2]
    resample_ohlcv يحذف الشمعة غير المكتملة — إذن [-1] هي آخر شمعة مغلقة
    """
    if len(df) < 22:
        return False
...
def check_rsi_stoch(df):
    ...
    ✅ إصلاح مزدوج:
    1) يستخدم [-1] وليس [-2]
    2) خُفف الشرط: RSI < 40 OR Stoch K < 25 (بدل AND الصعب)
    """
    ...
    # شرط مرن: يكفي واحد منهم
    return bool(rsi_val < 40 or stoch_k < 25)
...
def scan_symbol(symbol, entry_min, confirm_min, third_min, ec_api, t_api):
    ...
    # ✅ كل الشروط اتحققت — تحقق من alerted_keys لمنع التكرار
    ...
    with alerted_keys_lock:
        ...
        alerted_keys[key] = now  # ✅ حفظ وقت الإشارة
