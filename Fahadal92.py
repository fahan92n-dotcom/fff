...
✅ الإصلاح: Donchian Ribbon يطابق Pine Script

──────────────────────────────────────────────

def calc_donchian_trend(df, length=20):
"""
يحاكي Pine Script بالضبط:
trend = close > hh[1] ? 1 : close < ll[1] ? -1 : nz(trend[1])
الترند يستمر حتى ينكسر في الاتجاه المعاكس
"""
if len(df) < length + 2:
return []
...
trend[i] = trend[i - 1]  # يحافظ على الترند السابق
return trend

def check_donchian_ribbon(df, direction="green"):
"""
يتحقق من اتجاه Donchian Ribbon — أخضر أو أحمر
مطابق لـ TradingView Pine Script
"""
if len(df) < 22:
return False
...
list(ex.map(fn, syms))  # ✅ list() يكشف الأخطاء
