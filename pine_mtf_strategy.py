...
Replay runs on closed 5m bars (entry TF). This is a market-data simulation,
not live Binance account fills. Lookahead means historical results repaint.
"""
...
from binance_data import get_ohlcv_full, get_ohlcv_full_futures
...
def fetch_btc_1m(target=FETCH_1M_BARS):
    """BTCUSDT 1m OHLCV: Binance Futures, then Vision, then Spot."""
    df = get_ohlcv_full_futures(SYMBOL, "1m", target)
    if df is not None and not df.empty:
        log.info("Fetched %s 1m futures bars for %s", len(df), SYMBOL)
        return df, "futures"
    from pullback_bot.strategy import fetch_btc_1m_vision

    df = fetch_btc_1m_vision(target=target)
    if df is not None and not df.empty:
        log.info("Fetched %s 1m Vision bars for %s", len(df), SYMBOL)
        return df, "spot-vision"
    df = get_ohlcv_full(SYMBOL, "1m", target)
    if df is not None and not df.empty:
        log.info("Fetched %s 1m spot bars for %s", len(df), SYMBOL)
        return df, "spot"
...
def map_htf_lookahead(chart, htf, columns):
...
def build_chart(raw_1m, chart_minutes=CHART_TF):
    ...
    main_map = map_htf_lookahead(
        chart, main, ["smi", "macd", "hist", "trend", "ema50", "close", "rsi"]
    )
    confirm_map = map_htf_lookahead(chart, confirm, ["macd", "hist", "rsi"])
    out = chart.copy()
...
def scan_week(raw_1m=None, now=None, days=WEEK_DAYS, market=None):
    if raw_1m is None:
        log.info("Fetching %s 1m from Binance...", SYMBOL)
        raw_1m, market = fetch_btc_1m()
    if raw_1m is None or raw_1m.empty:
        raise RuntimeError("No OHLCV from Binance")
    ...
    result["market"] = market or "unknown"
...
def _side_ar(signal_type):
    return "شراء" if signal_type == "buy" else "بيع"


def _mark_ar(outcome):
    if outcome == "win":
        return "رابحة"
    if outcome == "loss":
        return "خاسرة"
...
def format_trade_line(trade):
    when = trade["fill_ts"].strftime("%Y-%m-%d %H:%M")
    side = _side_ar(trade["type"])
    mark = _mark_ar(trade["outcome"])
    line = (
    ...
    if trade["exit_ts"] is not None and trade["exit_price"] is not None:
        exit_when = trade["exit_ts"].strftime("%Y-%m-%d %H:%M")
        line += f" | خروج {exit_when} @ {trade['exit_price']:.2f}"
    return line
...
def format_report(result):
    ...
    lines = [
        f"{result.get('symbol', SYMBOL)} | 15m → 45m → 5m | آخر {WEEK_DAYS} أيام",
        f"السوق: {market}",
    ...
    if wins:
        lines.extend(format_trade_line(trade) for trade in wins)
    else:
        lines.append("— لا توجد صفقات رابحة")
    lines.append("")
    lines.append("الصفقات الخاسرة:")
    if losses:
        lines.extend(format_trade_line(trade) for trade in losses)
    else:
        lines.append("— لا توجد صفقات خاسرة")
    if opens:
        lines.append("")
        lines.append("صفقات لا تزال مفتوحة:")
        lines.extend(format_trade_line(trade) for trade in opens)
