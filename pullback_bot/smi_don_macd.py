"""SMI sat + 3× Donchian AND 3× MACD confirm. No EMA60, no RSI, no halt.

  Main SMI sat owns the side (largest main cancels smaller).
  Confirm TF = 3× main: Donchian and MACD must both agree
    (buy: Donchian green AND MACD histogram > 0;
     sell: Donchian red AND MACD histogram < 0).
  Entry TF: after confirm is aligned, wait until the combined
  Donchian+MACD gate is unmet, then enter when both hold.
"""
...
# main, confirm (3×), entry, win_pct, loss_pct, group
LEVELS = (
...
# MACD warmup on 450m needs ~200 bars ≈ 63 days before the scan window.
MIN_1M_BARS = 150_000
...
def _smi_features(df_1m, minutes):
...
def _don_macd_features(df_1m, minutes):
    """Donchian hue + MACD histogram sign on the same resampled frame."""
    df = resample_ohlcv_closed(df_1m, minutes)
    if df.empty or len(df) < max(WARMUP_MACD, DONCHIAN_DLEN + 2):
        return None
    ...
    return pd.DataFrame(
        {
            ...
            "don_green": don_arr == 1,
            "don_red": don_arr == -1,
            "macd_green": hist_arr > 0,
            "macd_red": hist_arr < 0,
        }
    )


def _merge_features(smi_feat, don_macd_feat):
    if smi_feat is None:
        return don_macd_feat
    if don_macd_feat is None:
        return smi_feat
    out = smi_feat.copy()
    for col in ("don_green", "don_red", "macd_green", "macd_red"):
        out[col] = don_macd_feat[col].to_numpy()
    return out
...
def _precompute_stepped(frame_data, grid):
    ...
    for minutes, feat in frame_data.items():
        ...
        if "sell_sat" in feat.columns:
            ...
            row["buy_sat"] = _bool_step(ends, feat["buy_sat"].to_numpy(), grid)
        if "don_green" in feat.columns:
            row["don_green"] = _bool_step(ends, feat["don_green"].to_numpy(), grid)
            row["don_red"] = _bool_step(ends, feat["don_red"].to_numpy(), grid)
        if "macd_green" in feat.columns:
            row["macd_green"] = _bool_step(ends, feat["macd_green"].to_numpy(), grid)
            row["macd_red"] = _bool_step(ends, feat["macd_red"].to_numpy(), grid)
        stepped[minutes] = row
    return stepped
...
def _scan_side(side, stepped, entry_data, grid, start, end, raw_1m, symbol):
    """Main SMI sat + 3× Donchian AND MACD confirm + combined entry reclaim."""
    is_sell = side == "sell"
    ...
    for level_idx, (main, confirm, entry, win_pct, loss_pct, group) in enumerate(LEVELS):
        ...
        confirm_macd = (
            confirm_feat[macd_key]
            if confirm_feat is not None
            else np.zeros(n_grid, dtype=bool)
        )
        owned = (active == level_idx) & confirm_don & confirm_macd
        ...
        for row_i, candle_end in enumerate(ends):
            ...
            don_green = bool(entry_df["don_green"].iloc[row_i])
            don_red = bool(entry_df["don_red"].iloc[row_i])
            macd_green = bool(entry_df["macd_green"].iloc[row_i])
            macd_red = bool(entry_df["macd_red"].iloc[row_i])
            price = float(entry_df["close"].iloc[row_i])
            don_ok = don_red if is_sell else don_green
            macd_ok = macd_red if is_sell else macd_green
            holds = don_ok and macd_ok
            cleared = not holds
...
def scan_symbol(symbol, *, days=MONTH_DAYS, now=None, raw_1m=None):
    ...
    if raw_1m is None:
        target = max(MIN_1M_BARS, int(days) * 1440 + 100_000)
    ...
    for minutes in needed:
        if minutes in entry_frames:
            entry_data[minutes] = _don_macd_features(raw_1m, minutes)
        smi_feat = _smi_features(raw_1m, minutes) if minutes in main_frames else None
        don_macd_feat = (
            _don_macd_features(raw_1m, minutes) if minutes in confirm_frames else None
        )
        merged = _merge_features(smi_feat, don_macd_feat)
...
def format_report(result):
    if not result.get("ready"):
        return ["⚠️ تعذر فحص SMI + Donchian + MACD."]
    ...
    header = (
        f"🗓️ <b>SMI + Donchian 3× و MACD 3× معًا (بدون EMA60/RSI) — آخر {days} يومًا</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"العملات: <code>{html_escape(symbols)}</code>\n"
        f"الفترة: <code>{start}</code> → <code>{end}</code> UTC\n"
        "الرئيسي: تشبع SMI فقط. الأكبر يلغي الأصغر.\n"
        "التأكيد 3×: Donchian و MACD لازم يتفقان (أخضر شراء / أحمر بيع).\n"
        "الدخول: بعد التأكيد، ننتظر الشرط المشترك ينطفئ ثم ندخل عند تحقق الاثنين.\n"
    )
    if failed:
...
def main():
    ...
    log.info("Scanning %s for %s days (Donchian AND MACD 3×)...", ",".join(SYMBOLS), MONTH_DAYS)
